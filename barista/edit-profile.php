...
if($ukuran < 1044070){     
    if (!empty($filename)){ 
        ...
        mysqli_query($koneksi, "UPDATE dt_user SET id_user='$id_user', nama='$nama', alamat='$alamat', telepon='$telepon', jabatan='$jabatan',  username='$username', password='$password', foto='$xx' WHERE id_user='$id_user'") or die(mysqli_error($koneksi));
        header("location:profil.php?alert=berhasil");
        }elseif(empty($filename)){
            move_uploaded_file($temp, '../gambar/'.$hidden1);
             mysqli_query($koneksi, "UPDATE dt_user SET id_user='$id_user', nama='$nama', alamat='$alamat', telepon='$telepon', jabatan='$hidden2',  username='$username', password='$password', foto='$hidden1' WHERE id_user='$id_user'") or die(mysqli_error($koneksi));
        ob_clean();
